...
@frappe.whitelist()
def get_recent_trans(search_text=None, limit=20):
    """
    ...
    if not search_text or not search_text.strip():
        ...
        if "Till Operator" in frappe.get_roles():
            conditions.append(f"docstatus = 1 AND DATE(modified) = '{today()}' AND (owner = '{session_user}' OR withdrawn_by = '{frappe.session.user}')")
            if conditions:
